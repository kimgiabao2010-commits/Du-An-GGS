...
export class WsCommandServer extends EventEmitter {
    ...
    constructor(port, options = {}) {
        ...
        this.wss = new WebSocketServer({
            ...
            verifyClient: (info, done) => {
                ...
                info.req.asqIdentity = claims;
                done(true);
            }
        });
        this.wss.on('error', error => this.emit('server:error', error));
        this.wss.on('connection', (ws, request) => {
            const identity = request.asqIdentity;
            ...
            if (this.clients.has(connectionId)) {
                ws.close(1008, 'Duplicate identity');
                return;
            }
            ...
            ws.on('message', buffer => {
                ...
                try {
                    ...
                    if (!data || !['COMMAND', 'EVIDENCE', 'RESULT', 'STATUS'].includes(data.type) ||
                        ...
                        seen.has(data.message_id)) {
                        ws.close(1008, 'Invalid or replayed message');
                        return;
                    }
                    if (seen.size >= 10000) {
                        ws.close(1008, 'Session message limit');
                        return;
                    }
                    seen.add(data.message_id);
                    if (data.payload.action === 'register_agent') {
                        if (data.payload.agentId !== identity.agentId)
                            ws.close(1008, 'Identity mismatch');
                        return;
                    }
                    if (data.type === 'COMMAND' && (identity.role !== 'CISO_Admin' ||
                        !identity.permissions.includes('CONTROL'))) {
    ...
    sendToAgent(agentId, payload) {
        ...
        if (!ws || ws.readyState !== WebSocket.OPEN || ws.bufferedAmount > 1048576)
            return false;
